...
def load_probe(path: str | Path) -> ActivationProbe:
    with np.load(path, allow_pickle=False) as data:
        ...
        probe = ActivationProbe(
            ...
            feature_mode=(
                str(data["feature_mode"]) if schema_version >= 4 else "pooled"
            ),
    ...
    if not (
        ...
        and probe.feature_mode in {"pooled", "outer_contrast"}
...
def score_examples(
    ...
    *,
    ...
    pool_width_tokens = tuple(probe.pool_widths)
    feature_mode = str(probe.feature_mode)
            ...
            with model.session(remote=True):
                ...
                for positions in batches:
                    ...
                    with model.trace(
                        ...
                        pooled = torch.cat(per_layer, dim=-1)
                        if feature_mode == "outer_contrast":
                            features = torch.cat(
