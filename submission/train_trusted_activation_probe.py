...
from activation_detector import encode_examples, is_transient_error, length_batches
...
def extract_features(
    ...
    print(f"  packed {len(encoded)} rows into {len(batches)} batches")
            ...
            with model.session(remote=True):
                ...
                for positions in batches:
                    ...
                    with model.trace(
                        ...
                        tracer.stop()
                saved_features = torch.cat(pieces).cpu().save()
            break
    ...
    sorted_features = saved_features.numpy()
    features = np.empty_like(sorted_features)
    features[np.asarray(order)] = sorted_features
    return features
